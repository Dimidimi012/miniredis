...
static int read_all(char *buf, size_t n) {
    size_t got = 0;
    while (got < n) {
        ssize_t r = recv(conn, buf + got, n - got, 0);
        if (r <= 0) return -1;
        got += (size_t)r;
    }
    return 0;
...
static char *read_line(void) {
    ...
    for (;;) {
        char ch;
        if (recv(conn, &ch, 1, 0) != 1) return NULL;
        if (n + 1 >= sizeof(buf)) return NULL;
        if (ch == '\r') {
            char nl;
            if (recv(conn, &nl, 1, 0) != 1 || nl != '\n') return NULL;
            buf[n] = '\0';
            return buf;
        }
        buf[n++] = ch;
...
static char *read_bulk(void) {
    ...
    if (read_all(payload, (size_t)n) != 0) {
        free(payload);
        failures++;
        return NULL;
    }
    ...
    char crlf[2];
    if (read_all(crlf, 2) != 0 || crlf[0] != '\r' || crlf[1] != '\n') {
        free(payload);
        failures++;
        return NULL;
    }
...
int main(int argc, char **argv) {
    ...
    /* Connect with retry; the server may still be starting. */
    int attempts = 50;
    while (attempts-- > 0) {
        conn = socket(AF_INET, SOCK_STREAM, 0);
        if (connect(conn, (struct sockaddr *)&addr, sizeof(addr)) == 0) break;
        close(conn);
        conn = -1;
        usleep(100 * 1000);
    }
